    ...
    def step(self, agent, runs, env, seeds):                                   
        ...
        for run in range(runs):
            ...
            for _ in range(self.max_step): # 250 steps .     instead of (for _ = 1, _ < n ,_ ++ )
                ...
                mu = mu.detach().numpy() # don't matter to gradient, don't record operations on this tensor
                action = mu #action is output of the system, in numpy format, with no recording (detached)

                # action is matrix of 2 * something
                action[0][1] = action[0][1] * np.pi / 4  # multiply by pi/4 . pi/4 = 45 degrees. something with direction ... 
            ...
            if not done:
                r += np.sqrt((env.goal[0] - env.sim.car[0])**2 + (env.goal[1] - env.sim.car[1])**2) * 10 * 3 # you do same as rew == -99
    ...
    def train(self, num_agents, generations, top_limit, file):
        ...
        for generation in range(generations):
            ...
            if generation % 10 == 0:
                ...
                torch.save(agents[elite_index].state_dict(), 'envs/models/' + file + '_{}'.format(generation))
